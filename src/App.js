  ...
  const fetchWeather = async (e) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 404) {
        setError("Ciudad no encontrada. Por favor, ingresa otra ciudad.");
      } else {
        setError("Ocurrió un error al obtener el clima. Inténtalo de nuevo más tarde.");
      }
    } finally {
      setLoading(false);
    }
